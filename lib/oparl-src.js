...
(function (window, document) {
	...
	OParl.Handler.handleSystem = function (json, callback) {
		...
		if (json.created) obj.created = new Date(Date.parse(json.created));
		if (json.modified) obj.modified = new Date(Date.parse(json.modified));
	...
	OParl.Handler.handleBody = function (json, callback) {
        ...
        if (json.created) obj.created = new Date(Date.parse(json.created || null));
        if (json.modified) obj.modified = new Date(Date.parse(json.modified || null));
	...
	OParl.Handler.handleMeeting = function (json, callback) {
		...
		if (json.start) obj.start = new Date(Date.parse(json.start || null));
		if (json.end) obj.end = new Date(Date.parse(json.end || null));
    ...
    OParl.Handler.handlePaper = function (json, callback) {
        ...
        obj.superordinatedPaper = {
			get: OParl.Util.callObjectArrayParser,
			data: json.superordinatedPaper || []
		};

        obj.subordinatedPaper = {
			get: OParl.Util.callObjectArrayParser,
        ...
        obj.originatorPerson = {
			get: OParl.Util.callObjectArrayParser,
			data: json.originatorPerson || []
		};

        obj.underDirectionOf = {
			get: OParl.Util.callObjectArrayParser,
			data: json.underDirectionOf || []
		};

        obj.originatorOrganization = {
			get: OParl.Util.callObjectArrayParser,
    ...
    OParl.Handler.handleFile = function (json, callback) {
		var obj = {};

        obj.objectType = 'oparl:File';
        ...
        obj.size = json.size || '';
        obj.sha1Checksum = json.sha1Checksum || '';
        obj.text = json.text || '';

        if (json.date) obj.date = new Date(Date.parse(json.date));

        obj.accessUrl = json.accessUrl || '';
        if (json.downloadUrl) obj.downloadUrl = json.downloadUrl || '';
        obj.fileLicense = json.fileLicense || '';

        obj.masterFile = {
            get: OParl.Util.callObjectParser,
            data: json.masterFile || null
        };

        obj.derivativeFile = {
			get: OParl.Util.callObjectArrayParser,
			data: json.derivativeFile || []
		};
    ...
    OParl.Util.registerNamespace('https://schema.oparl.org/1.0/File', OParl.Handler.handleFile);
	OParl.Util.registerNamespace('https://schema.oparl.org/1.1/File', OParl.Handler.handleFile);
